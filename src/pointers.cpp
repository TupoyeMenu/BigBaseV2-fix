...
namespace big
{
	pointers::pointers()
	{
		...
		main_batch.add("Fix vectors", "83 79 18 00 48 8B D1 74 4A FF 4A 18 48 63 4A 18 48 8D 41 04 48 8B 4C CA", -1, -1, eGameBranch::LEGACY, [this](memory::handle ptr) {
		...
		main_batch.add("CGameScriptHandlerMgr", "48 8B 0D ? ? ? ? 4C 8B CE E8 ? ? ? ? 48 85 C0 74 05 40 32 FF", -1, -1, eGameBranch::LEGACY, [this](memory::handle ptr) {
		...
		// We already check this in main.cpp, why not set it there?
