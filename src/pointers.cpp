...
namespace big
{
	pointers::pointers()
	{
		...
		main_batch.add("Game state", "83 3D ? ? ? ? ? 0F 85 ? ? ? ? BA ? 00", -1, -1, eGameBranch::ENCHANCED, [this](memory::handle ptr) {
			m_game_state = ptr.add(2).rip().add(1).as<eGameState*>();
		});
		...
		main_batch.add("Is session started", "0F B6 05 ? ? ? ? 0A 05 ? ? ? ? 75 2A", -1, -1, eGameBranch::ENCHANCED, [this](memory::handle ptr) {
			m_is_session_started = ptr.add(3).rip().as<bool*>();
		});
		...
		main_batch.add("Ped factory", "C7 40 30 03 00 00 00 48 8B 0D", -1, -1, eGameBranch::ENCHANCED, [this](memory::handle ptr) {
		...
		main_batch.add("Network player manager", "75 0E 48 8B 05 ? ? ? ? 48 8B 88 F0 00 00 00", -1, -1, eGameBranch::ENCHANCED, [this](memory::handle ptr) {
		...
		main_batch.add("Init Native Tables", "EB 2A 0F 1F 40 00 48 8B 54 17 10", -1, -1, eGameBranch::ENCHANCED, [this](memory::handle ptr) {
		...
		main_batch.add("Script threads", "BE 40 5D C6 00", -1, -1, eGameBranch::ENCHANCED, [this](memory::handle ptr) {
		...
		main_batch.add("Script programs", "89 46 38 48 8B 0D ? ? ? ? 0F", -1, -1, eGameBranch::ENCHANCED, [this](memory::handle ptr) {
		...
		main_batch.add("Script globals", "48 8B 8E B8 00 00 00 48 8D 15 ? ? ? ? 49 89 D8", -1, -1, eGameBranch::ENCHANCED, [this](memory::handle ptr) {
		...
		main_batch.add("Script VM", "49 63 41 1C", -1, -1, eGameBranch::ENCHANCED, [this](memory::handle ptr) {
		...
		main_batch.add("HandlesAndPtrs", "0F 1F 84 00 00 00 00 00 89 F8 0F 28 FE 41", -1, -1, eGameBranch::ENCHANCED, [this](memory::handle ptr) {
		...
		main_batch.add("Queue Dependency", "0F 29 46 50 48 8D 05", -1, -1, eGameBranch::ENCHANCED, [this](memory::handle ptr) {
